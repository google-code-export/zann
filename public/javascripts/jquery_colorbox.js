...
(function ($, window) {
	...
	isIE = $J.browser.msie && !$J.support.opacity, // feature detection alone gave a false positive on at least one phone browser and on some development versions of Chrome.
	...
	function launch(elem) {
		...
		settings = $J.extend({}, $(element).data(colorbox));
	...
	publicMethod = $J.fn[colorbox] = $[colorbox] = function (options, callback) {
		var $this = this;
		...
		if (!$this[0] || $this.selector === undefined) { // detects $J.colorbox() and $J.fn.colorbox()
			$this = $('<a/>');
			options.open = true; // assume an immediate open
		}
		
		$this.each(function () {
			$(this).data(colorbox, $J.extend({}, $(this).data(colorbox) || defaults, options)).addClass(boxElement);
		});
	...
	publicMethod.prep = function (object) {
		...
		function setPosition(s) {
			...
			publicMethod.position(s, function () {
				...
				$J.event.trigger(event_complete);
